...
namespace nio2so.Voltron.PreAlpha.Protocol.PDU
{
    ...
    /// </summary>
    public class TSODBWrapperPDUHeader : ITSOVoltronSpecializedPDUHeader
    {
    ...
    [Obsolete]
    public class TSODBWrapperMessageSize
    {
    ...
    /// </summary>    
    public abstract class TSODBRequestWrapper : TSOVoltronSpecializedPacket<TSOVoltronDBWrapperField, TSODBWrapperPDUHeader>, ITSOVoltronAriesMasterIDStructure
    {
        /// <summary>
        ...
        /// <see cref="TSODBRequestWrapper"/> OR writing a <see cref="TSODBRequestWrapper"/> that has a <see cref="kMSGID"/>
        ...
        public static TSODBWrapperPDUHeader ReadDBPDUHeader(Stream BodyStream)
        {
            ...
            uint kMSG = BodyStream.ReadBodyDword();
        ...
        public override string ToString() => $"{kMSGID}->{GetDBWrapperName()}({GetParameterListString()})";

        public override string ToShortString(string Arguments = "") => $"{kMSGID}->{GetDBWrapperName()}";

        public string GetDBWrapperName() => $"{TSOPacketFormatCLSID}::{TSOSubMsgCLSID}";
