...
namespace nio2so.Voltron.PreAlpha.Protocol.Struct
{
    ...
    public class TSOHouseChunkPack : ITSOCustomDeserialize
    {
        ...
        private void PopulateChunks(Stream DataStream)
        {
            ...
            {
                //read next 4 bytes
                uint headerBytes = DataStream.ReadBodyDword(Endianness.BigEndian);
                //cast to enum
                ...
                if (!Enum.IsDefined(header))
                {
                    ...
                    headerBytes = DataStream.ReadBodyDword(Endianness.LittleEndian);
            ...
            while (DataStream.Position != DataStream.Length)
            {
                ...
                uint param1 = DataStream.ReadBodyDword(Endianness.LittleEndian);
                uint size = DataStream.ReadBodyDword(Endianness.LittleEndian);
