...
namespace nio2so.Formats.Terrain
{
    ...
    internal class TSOCityGeom
    {
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private static Task<Vector3> GetNormalAt(TSOCity City, int x, int y)
        {
        ...
        [Obsolete("Logic now included in GenerateCityGeometry function as of May 3rd, 2026")]
        public static async Task CalculateNormals(TSOCityMesh Mesh, TSOCity City)
        {
            await Parallel.ForEachAsync(Mesh.Vertices, async (vertCollection, cancellation) =>
            {
                await Parallel.ForEachAsync(vertCollection.Value, async (vert, cancellation) =>
                {
        ...
        /// <returns></returns>
        public async Task<TSOCityMesh> GenerateMeshGeometry(TSOCity CityData, bool VertexColorAtlasEnabled = true)
        {
                    ...
                    for (int i = yStart; i < yEnd; i++)
                    {
                        ...
                        { //where the magic happens B)
                          //get terrain type here
                            ...
                            var normal = await GetNormalAt(CityData, ex, i);
                            if (false)
                            {
                                var norm1 = await GetNormalAt(CityData, Math.Min(rXE, Math.Max(rXS, j)), i);
                                var norm2 = await GetNormalAt(CityData, Math.Min(rXE, Math.Max(rXS, j + 1)), i);
                                var norm3 = await GetNormalAt(CityData, Math.Min(rXE2, Math.Max(rXS2, j + 1)), Math.Min((imgSize - 1), i + 1));
                                var norm4 = await GetNormalAt(CityData, Math.Min(rXE2, Math.Max(rXS2, j)), Math.Min((imgSize - 1), i + 1));
                            }
                            //vertex pos
                            ...
                            // make geom vert
                            var vert1 = new GeomVertex(pos1, mapPos, new(normal), new(0, 0));
                            var vert2 = new GeomVertex(pos2, mapPos, new(normal), new(1, 0));
                            var vert3 = new GeomVertex(pos3, mapPos, new(normal), new(1, 1));
                            var vert4 = new GeomVertex(pos4, mapPos, new(normal), new(0, 1));
