...
namespace nio2so.Voltron.NewImproved.Protocol.Regulator
{
    ...
    /// </summary>
    [TSORegulator("TSONewImprovedSplitBufferPDUProtocol")]
    internal class SplitBufferPDUProtocol : TSOProtocol
    {
        internal class SplitBufferPDUThreadContext : IDisposable
        {
            ...
            internal void DoProtocolOnThread(TSOPDUFactoryServiceBase FactoryService, TSOVoltronPacket PDU, out TSOVoltronPacket? UnsplitPacket)
            {
                ...
                _recvPDUs++;
                var splitBuffer = (TSONewImprovedSplitBufferPDU)PDU;
                if (!IsUnpacking)
                { // START UNPACKING                    
        ...
        [TSOProtocolHandler((uint)TSO_NewImproved_VoltronPacketTypes.SplitBufferPDU)]
        public void DoProtocol(TSOVoltronPacket PDU)
        {
            ...
            if (!_threads.TryGetValue(ID, out SplitBufferPDUThreadContext? context) || context == null)
                throw new Exception($"{nameof(TSONewImprovedSplitBufferPDU)} cannot create a new context for the thread: {ID}");
            ...
            { // decompressed a PDU ... insert it into this voltron aries frame
                ...
                LogConsole($"Inserted the {DesplitPDU}\n\nFrom {context._recvPDUs} {nameof(TSONewImprovedSplitBufferPDU)}s ... ({context._recvBytes} bytes)");
